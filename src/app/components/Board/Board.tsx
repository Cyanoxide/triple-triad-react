...
const Board: React.FC<BoardProps> = ({ className }) => {
    ...
    const updateScore = () => {
        const redScore = board.flat().filter(entry => entry?.[1] === "red").length + enemyCards.length
        const blueScore = board.flat().filter(entry => entry?.[1] === "blue").length + playerCards.length
